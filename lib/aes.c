...
static const uint8_t shift_rows_table_inv[] = {0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2, 15, 12, 9, 6, 3};
...
// Galois field manipulation lookup tables
...
void aes_set_key(const uint8_t *key_in, AES_KEY *key_out, size_t len) {
    if (len == 16) {
        // 128 bit key
        key_out->size = len;
        aes_expand_key(key_out->round_keys, key_in);
    } else if (len == 24) {
        // 192 bit key
        key_out->size = len;
    } else if (len == 32) {
...
void aes_decrypt(const uint8_t *in, uint8_t *out, const AES_KEY *key) {
    ...
    add_round_key(out, key->round_keys, 10);
    shift_rows_inv(out);
    sub_bytes_inv(out, AES_BLOCK_SIZE);
