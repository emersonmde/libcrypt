 ...
 * Tests for aes.c
 *
 * Tests aes_expand_key(), aes_encrypt(), aes_decrypt(), aes_cbc_encrypt(), and aes_cbc_decrypt()
 */
...
int test_aes_expand_key() {
    ...
    if (memcmp(expanded_key, expected_key, 176) == 0) {
        ...
        printf("Expected expanded key: ");
        print_hex(expected_key, sizeof(expected_key));

        printf("Actual expanded key: ");
...
int test_aes_cbc_encrypt() {
    AES_KEY aes_key;
    unsigned char plaintext[] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd,
                                 0xee, 0xff};
    uint8_t key[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
...
int test_aes_cbc_decrypt() {
    AES_KEY aes_key;
    unsigned char ciphertext[] = {0xbe, 0xcd, 0x41, 0xa3, 0x6b, 0x0e, 0x24, 0xa1, 0xed, 0xb1, 0x03, 0x04, 0xce,
    ...
    aes_set_key(key, &aes_key, 16);
    aes_cbc_decrypt(ciphertext, plaintext, sizeof(plaintext), &aes_key, iv);
